...
class CMB2_JS {
	...
	protected static function colorpicker_frontend() {
		...
		wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
			'clear'         => esc_html__( 'Clear', 'opal-service' ),
			'defaultString' => esc_html__( 'Default', 'opal-service' ),
			'pick'          => esc_html__( 'Select Color', 'opal-service' ),
			'current'       => esc_html__( 'Current Color', 'opal-service' ),
		) );
	}
		...
		$l10n = array(
			...
			'defaults'          => array(
				...
				'date_picker'  => array(
					'changeMonth'     => true,
					'changeYear'      => true,
					'dateFormat'      => _x( 'mm/dd/yy', 'Valid formatDate string for jquery-ui datepicker', 'opal-service' ),
					'dayNames'        => explode( ',', esc_html__( 'Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday', 'opal-service' ) ),
					'dayNamesMin'     => explode( ',', esc_html__( 'Su, Mo, Tu, We, Th, Fr, Sa', 'opal-service' ) ),
					'dayNamesShort'   => explode( ',', esc_html__( 'Sun, Mon, Tue, Wed, Thu, Fri, Sat', 'opal-service' ) ),
					'monthNames'      => explode( ',', esc_html__( 'January, February, March, April, May, June, July, August, September, October, November, December', 'opal-service' ) ),
					'monthNamesShort' => explode( ',', esc_html__( 'Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec', 'opal-service' ) ),
					'nextText'        => esc_html__( 'Next', 'opal-service' ),
					'prevText'        => esc_html__( 'Prev', 'opal-service' ),
					'currentText'     => esc_html__( 'Today', 'opal-service' ),
					'closeText'       => esc_html__( 'Done', 'opal-service' ),
					'clearText'       => esc_html__( 'Clear', 'opal-service' ),
				),
				'time_picker'  => array(
					'timeOnlyTitle' => esc_html__( 'Choose Time', 'opal-service' ),
					'timeText'      => esc_html__( 'Time', 'opal-service' ),
					'hourText'      => esc_html__( 'Hour', 'opal-service' ),
					'minuteText'    => esc_html__( 'Minute', 'opal-service' ),
					'secondText'    => esc_html__( 'Second', 'opal-service' ),
					'currentText'   => esc_html__( 'Now', 'opal-service' ),
					'closeText'     => esc_html__( 'Done', 'opal-service' ),
					'timeFormat'    => _x( 'hh:mm TT', 'Valid formatting string, as per http://trentrichardson.com/examples/timepicker/', 'opal-service' ),
					'controlType'   => 'select',
					'stepMinute'    => 5,
				),
			),
			'strings' => array(
				'upload_file'  => esc_html__( 'Use this file', 'opal-service' ),
				'upload_files' => esc_html__( 'Use these files', 'opal-service' ),
				'remove_image' => esc_html__( 'Remove Image', 'opal-service' ),
				'remove_file'  => esc_html__( 'Remove', 'opal-service' ),
				'file'         => esc_html__( 'File:', 'opal-service' ),
				'download'     => esc_html__( 'Download', 'opal-service' ),
				'check_toggle' => esc_html__( 'Select / Deselect All', 'opal-service' ),
			),
		);
